...
export default class GroupController {
    ...
    async addGroup(request: FastifyRequest): Promise<jsend.JSendObject> {
        ...
        const i_group = group.i_group;
        if (!request.body.allowed_all && !!request.body.allowed_devices) {
            for (let index = 0; index < request.body.allowed_devices.length; index++) {
                const i_device = request.body.allowed_devices[index].i_device;
    ...
    async updateGroup(
        ...
        if (!group_info.allowed_all && !!allowed_devices) {
            GroupDevicePermissions.bulkCreate(allowed_devices.map(({ i_device }) => ({ i_device, i_group })));
        }
